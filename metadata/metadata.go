...
The full claim must be retrievable through an HTTP multiaddr of the provider
which contains path segments of the form `{claim}`. To retrieve the claim,
...
func init() {
	...
	nodePrototypes[IndexClaimID] = indexClaimMetadata
	nodePrototypes[EqualsClaimID] = equalsClaimMetadata
